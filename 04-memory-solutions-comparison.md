...
> **Для кого:** команда разработки AI-ассистента
> **Цель:** выбрать платформу памяти с учётом механик Honcho, масштабируемости и лицензионной совместимости
...
**Готового решения на рынке нет — но на этапе прототипа это не проблема.** Мы проанализировали 20+ платформ для AI-памяти. Ни одна не реализует все 6 механизмов Honcho нативно. Лучшие покрывают 2-4 из 6.

**Рекомендация: оставаться на Honcho до product-market fit, затем мигрировать на mem0 + pgvector.** Honcho — единственное решение с 6/6 механик из коробки. На этапе прототипа (13 юзеров) нет смысла строить своё. Когда масштаб потребует замены (AGPL-блокер, >1K юзеров), основа — mem0 (extraction + dedup, Apache 2.0) с soft temporal metadata вместо отдельного graph database.

**Graphiti убран из рекомендации** после deep dive: не production-ready (12 сек/episode, баги), temporal tracking решается проще (Dreaming pipeline + metadata), Zep убил Community Edition, 1.68x token cost за решение проблемы, которой у компаньона нет.

**Почему не «просто взять mem0 сейчас»:** mem0 покрывает 2/6 механик Honcho. Dreaming, Peer Card, Dialectic Chat, Prefetch — отсутствуют. Дописать это = ~70% работы. На этапе прототипа Honcho даёт всё бесплатно.

### Ключевые выводы

- **Honcho (AGPL) — лучший для прототипа**, блокер для enterprise без отдельного соглашения
- **mem0 (Apache 2.0) — лучший для масштабирования**, но нужна значительная обвязка (Dreaming, Peer Card, Dialectic)
- **Graphiti не нужен**: temporal tracking реализуется через Dreaming pipeline + metadata полей (`created_at`, `previous_value`)
- **Vendor LLM дороже GPT-4o-mini в 15 раз** ($2.20 vs $0.15 за 1M токенов). Cost argument не работает на API, только на self-hosted
- **Тренд в сообществе**: минимализм (skill.md + memory.md) побеждает фреймворки. Разработчики строят память из простых компонентов, а не берут готовые платформы

### Рекомендация по стадиям

| Стадия | Решение | Обвязка | Cost/мес |
|--------|---------|---------|----------|
| **MVP (сейчас, 1-1K)** | **Honcho** | 0 — всё из коробки | $30 |
| **Пилот (1K-100K)** | **mem0 + soft temporal metadata** | Dreaming, Peer Card, Prefetch — custom | $500 |
| **Scale (100K+)** | **Custom extraction + pgvector** | Batch processing, self-hosted LLM | $10-30K |
...
**Target:** 47.5M подписчиков Плюс = потенциальная аудитория AI-компаньона. Реалистичный DAU: 5-10% = 2.5-5M ежедневных пользователей.
...
#### Phase 0: Доказать ценность (сейчас → 2 мес)

**Вопрос фазы:** Хочет ли юзер общаться с AI-компаньоном, который помнит его вкусы?

| Что делаем | Зачем | Как измеряем |
|-----------|-------|-------------|
| Довести профили до 100% юзеров | Без профиля персонализация невозможна | 13/13 с peer card |
| Собрать обратную связь | Понять что нравится, что раздражает | 5+ интервью |
| Определить North Star Metric | Без метрики не понять, работает ли продукт | NSM определена |

**Стек:** Honcho (SaaS, бесплатно). Менять инфру рано — сначала ценность.
**Команда:** 0 backend (founder only)
**Gate → Phase 1:** retention >30% DAU/MAU, NPS >40

#### Phase 1: Закрытый пилот (2-6 мес)

**Вопрос фазы:** Работает ли это за пределами 13 друзей? Какие фичи памяти реально двигают engagement?

| Что делаем | Зачем | Как измеряем |
|-----------|-------|-------------|
| 10K юзеров (Видеостриминг beta) | Валидация на незнакомых юзерах | Retention W1, W4 |
| A/B: память ON vs OFF | Доказать что память = ценность, а не гипотеза | Δ retention, Δ session length |
| Приоритизация механик | Не все 6 механик Honcho нужны сразу. Какие 3 двигают engagement? | Impact каждой механики на NSM |
| Миграция на собственную инфру | AGPL Honcho — блокер для enterprise-платформы | Работающий self-hosted стек |

**Стек:** mem0 (extraction + dedup) + Postgres. Начинаем с 3 механик из 6 — Extraction, Peer Card, Prefetch. Dreaming и Dialectic откладываем до доказательства ценности.
**Команда:** 1 backend, 1 PM
**Cost:** ~$500/мес (infra) + salary
**Gate → Phase 2:** W4 retention >20%, доказанный Δ retention память ON vs OFF

#### Phase 2: Масштабирование (6-12 мес)

**Вопрос фазы:** Может ли система обслужить 1M+ юзеров без деградации качества?

| Что делаем | Зачем | Как измеряем |
|-----------|-------|-------------|
| Batch extraction | При 5M msg/day per-message LLM вызовы экономически невозможны | Cost/user/month <$0.01 |
| Dreaming pipeline | Офлайн-анализ: сдвиги вкусов, паттерны, противоречия | Extraction accuracy >90% |
| Интеграция КП + Музыка + Книги | Cold start: не ждать 10 сообщений — знать вкусы из истории сервисов | Time to first personalized response <1 мин |
| Cross-service персонализация | "Ты смотрел X на КП → тебе зайдёт Y в Музыке" | Cross-service recs CTR |

**Стек:** Custom extraction + pgvector + self-hosted LLM (Qwen3-8B при >1M users)
**Команда:** 2-3 backend, 1 ML, 1 PM
**Cost:** $10-30K/мес
**Gate → Phase 3:** prefetch P95 <200ms, 1M+ users без деградации

#### Phase 3: Продуктовый масштаб (12+ мес)

**Вопрос фазы:** Увеличивает ли AI-компаньон retention Подписка (стриминговая платформа)?

| Что делаем | Зачем | Как измеряем |
|-----------|-------|-------------|
| 47.5M users rollout | Массовый запуск | DAU/MAU >30% |
| Churn reduction | Бизнес-метрика: компаньон удерживает подписчиков Плюс | Δ churn vs baseline >15% |
| 152-ФЗ compliance | Юридический блокер для РФ | Аудит пройден |
| Multi-modal память | Обложки фильмов, фрагменты треков, скриншоты | Engagement с мультимедиа |

**Стек:** Custom memory platform
**Команда:** 6+ engineers
**Cost:** $40-75K/мес

### Ключевые принципы roadmap

1. **Каждая фаза отвечает на один вопрос.** Не "построим инфру", а "докажем что память = ценность"
2. **Gate между фазами — продуктовая метрика, не техническая.** Retention, не "сервис поднят"
3. **Не все 6 механик Honcho нужны сразу.** Для Phase 1 достаточно 3: Extraction + Peer Card + Prefetch. Dreaming добавляем когда доказали что базовая память двигает retention
4. **Инфра следует за продуктом, не наоборот.** Honcho → mem0 → custom — каждый переход оправдан продуктовой необходимостью, не техническим зудом
5. **Самое дорогое — custom orchestrator (~70% работы).** Оттягиваем его до Phase 2, когда точно знаем какие механики нужны
